document.addEventListener('DOMContentLoaded', () => {
    ...
    // Создание параметра
    let tagsList = document.querySelector('.filter__tags')
    function createElem(text){
        let li = document.createElement('li')
        li.className = 'filter__tags-item'
        li.innerHTML = `${text}<span class="filter__tags-item__cross"></span>`
        tagsList.append(li)
    }
    let choices = document.querySelectorAll('.main__filter_item')
    for(let i = 0; i < choices.length; i++){
        choices[i].addEventListener('click', function(){
            let text = this.querySelector('.main__filter_title').innerHTML
            createElem(text)
        })
    }
    // Удаление параметра
    document.querySelector('.filter__tags-item__cross').addEventListener('click', function(){
        console.log(this)      
    })
    ...
    let paginations = document.querySelector('.slider-pagination').querySelectorAll('.slider-pagination-item') // Точки для навигации(снизу)
    ...
    function changeSlide(forward) {
        let active = sliderTrack.querySelector('.slide.active').getAttribute('data-slide-index') - 1 // Ищем активный слайд и берём его номер
        ...
        // Перелистывание назад
            ...
            if(active == 0){
                ...
                sliderTrack.style.transform = `translate3D(-${slideWidth * (numOfSlides - 1)}px, 0, 0)`      
                paginations[active].classList.remove('active')
            ...
            // Условие для всех слайдов кроме первого
                ...
                sliderTrack.style.transform = `translate3D(-${slideWidth * active}px, 0, 0)`   
